...
export default async function WPAuditTaxonomy() {
  ...
  const categoryFields = categories && categories.length > 0 ? extractFields(categories[0]) : {};
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <pre style={{ background: '#eee', padding: 12, maxHeight: 300, overflow: 'auto' }}>{JSON.stringify(categories[0], null, 2)}</pre>
      <h2>Raw Data Example (Tag)</h2>
      <pre style={{ background: '#eee', padding: 12, maxHeight: 300, overflow: 'auto' }}>{JSON.stringify(tags[0], null, 2)}</pre>
    </div>
  );
}
